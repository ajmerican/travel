...
<?php
	...
	$sabre = new Sabre();
	$path = '/shop/flights/fares?origin='.$origin.'&destination='.$destination.'&departuredate='.$dateStart.'&lengthofstay=5';
	$response = $sabre->get($path);
$lib->debug($response);
	$response = $sabre->flightsFaresResponse($response);
...
<div class="price-section">
	<div class="container">
		<div class="flight">
			<h1><i class="icon-airplane"></i> <?php echo $flight['DepartureAirport']['LocationCode']; ?> to <?php echo $flight['ArrivalAirport']['LocationCode']; ?><br/><span><?php echo date('l, j F, Y', strtotime($flight['DepartureDateTime'])); ?></span></h1>
		</div>
		<div class="row">
		<div class="col-lg-10 col-md-10 col-sm-12 col-xs-12 ass">
		<div class="arrival-time">
			<ul>
					<li class="one"><b><?php echo date('h:i', strtotime($flight['DepartureDateTime'])); ?></b> <span><?php echo $flight['DepartureAirport']['LocationCode']; ?></span> <img src="data:image/png;base64,"></li>
